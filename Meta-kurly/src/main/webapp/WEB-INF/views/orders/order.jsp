	...
	<div
					...
					<div
						...
						<div style="padding: 4px 0 0;">
							...
							<span
								...
								<fmt:formatNumber type="number" maxFractionDigits="3" value="${product.price * product.quantity}" /><em
				...
				<div
					...
					<div
						...
						<strong
							...
							<c:forEach items="${products}" var="product">
								<c:set var="totalPrice" value="${totalPrice + product.totalPrice}"/>
							</c:forEach>
							
							<fmt:formatNumber type="number" maxFractionDigits="3" value="${totalPrice}" /><em
				...
				<div
					...
					<div
						...
						<strong
							...
							<p id="finalPrice" style="display: inline"><c:out value="${totalPrice}" /><%-- <fmt:formatNumber type="number" maxFractionDigits="3" value="${product.totalPrice}" /> --%></p><em
							style="margin: 0; padding: 0; border: 0; box-sizing: border-box; font-size: 14px; line-height: 22px; font-family: '맑은고딕', 'Malgun Gothic', 'dotum', sans-serif; letter-spacing: -1px; display: inline-block; margin-left: 3px; font-style: normal; font-weight: bold; font-size: 12px; vertical-align: 2px; letter-spacing: -1px;">원</em>
						</span>
					</div>
				</div>
		...
		<form id="paymentForm" action="/orders/payment" method="post">
			...
			<c:forEach items="${products}" var="product" varStatus="status">
				<input type="hidden" name="orderProductList[${status.index}].p_id" value="${product.p_id}"/>
				<input type="hidden" name="orderProductList[${status.index}].quantity" value="${product.quantity}"/>
				<input type="hidden" name="orderProductList[${status.index}].totalPrice" value="${product.totalPrice}"/>
			</c:forEach>
<%-- 			<input type="hidden" name="p_id" value="${product.p_id}"/>
...
<script type="text/javascript">
	$("#usePointNum").change(function(e){
		...
		if(usePoint > price || usePoint > ${member.point}){
			...
			var finalPrice = parseInt(${totalPrice}) - usePoint;
